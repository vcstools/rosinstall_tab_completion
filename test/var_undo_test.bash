...
_runtest () {
    ...
    if [ "$TEST_VAR_VAR_UNDO_BACKUP" != "$_test_core" ]; then
    ...
    if [ -z $2 ]; then
        ...
        if [ -n "$_test_prefix" -a "$TEST_VAR_VAR_UNDO_PREFIX" != "$_test_prefix" ]; then
            echo $1 e4 "$_test_prefix" != "$TEST_VAR_VAR_UNDO_PREFIX"; exit 1
        fi
        if [ -n "$_test_suffix" -a "$TEST_VAR_VAR_UNDO_SUFFIX" != "$_test_suffix" ]; then
            echo $1 e5 "$_test_suffix" != "$TEST_VAR_VAR_UNDO_SUFFIX"; exit 4
        fi
    fi
    if [ "$TEST_VAR_VAR_UNDO_PREFIX$_test_core$TEST_VAR_VAR_UNDO_SUFFIX" != "$_test_prefix$_test_core$_test_suffix" ]; then
        echo $1 e7 "$TEST_VAR_VAR_UNDO_PREFIX$_test_core$TEST_VAR_VAR_UNDO_SUFFIX" != "$_test_prefix$_test_core$_test_suffix"; exit 6
    fi
    if [ -n "$_test_core" -a "$TEST_VAR_VAR_UNDO_BACKUP" != "$_test_core" ]; then
        echo $1 e8 "$_test_core" != $TEST_VAR_VAR_UNDO_BACKUP; exit 5
    fi
    
    export TEST_VAR="$_test_addprefix$TEST_VAR$_test_addsuffix"
    if [ "$TEST_VAR" != "$_test_addprefix$_test_prefix$_test_core$_test_suffix$_test_addsuffix" ]; then
    ...
    if [ "$TEST_VAR" != "$_test_addprefix$_test_core$_test_addsuffix" ]; then
